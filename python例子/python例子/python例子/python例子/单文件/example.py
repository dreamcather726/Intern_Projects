...
class SerialWorker(QObject):
        ...
        xiaozhi_connect = pyqtSignal(str,str)  # 发送小智连接状态给主线程
        ...
        def _process_json_data(self, json_buffer):
            ...
            try:
                ...
                elif json_data.get("event") == "wifi_set":
                    print("收到WiFi设置请求")
                    self.xiaozhi_connect.emit("wifi_set","0")
                    # 这里可以根据需求添加处理wifi_set事件的逻辑
                    # 例如获取SSID和密码信息并进行连接
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.time_timer.start(30000)  # 3000毫秒 = 3秒
        # 创建小智问询定时器
        self.xiaozhi_timer = QTimer(self)
        self.xiaozhi_timer.timeout.connect(self.ask_xiaozhi_network)
        self.xiaozhi_timer.start(xiaozhi_asktime)  # 10000毫秒 = 10秒
    ...
    def init_labels(self):
        ...
        self.init_label(self.xiaozhi_status, "小智未连接", "rgb(128,128,128)")
        # 初始化大门和灯光设备标签
    ...
    def ask_xiaozhi_network(self):
        ...
        try:
            ...
            SerialWorker.send_data(SerialWorker,ask_json_str.encode('utf-8'))
    ...
    def update_xiaozhi_display(self,role,content):
        ...
        print(f"主线程JSON数据: 角色: {role}, 内容: {content}")
        # 更新小智状态标签
        self.init_label(self.xiaozhi_status, "小智已连接","rgb(186,128,0)" )
        if role=="assistant":
            self.init_label(self.assistant_label,f"{content}")
        elif role=="user":
            self.init_label(self.user_label,f"{content}")
...
def main():
    ...
    try:
        ...
        serial_worker.xiaozhi_connect.connect(main_window.update_xiaozhi_display)

...
class BlinkerWorker(QObject):
    ...
    def run(self):
        ...
        try:
            ...
            # 创建设备实例，设置实时数据处理函数
            device = Device(device_key, heartbeat_func=heartbeat_func)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, blinker_worker=None):
        super().__init__()        
        uic.loadUi('main.ui', self)##加载UI文件'/home/pi/aihome/main.ui'
    ...
    def update_voice_display(self, device_id,node,command):
        ...
        if device_id == 0x01:
            if node == 0x01:## 打开主卧灯光
                print("打开主卧灯光")
                self.init_label(self.hostroom_light_label, "开启", font=10, color="black")
            elif node == 0x02:## 关闭主卧灯光
                print("关闭主卧灯光")
                self.init_label(self.hostroom_light_label, "关闭", font=10, color="black")
            elif node == 0x03:## 打开次卧灯光
                print("打开次卧灯光")
                self.init_label(self.secound_light_label, "开启", font=10, color="black")
            elif node == 0x04:## 关闭次卧灯光
                print("关闭次卧灯光")
                self.init_label(self.secound_light_label, "关闭", font=10, color="black")
            elif node == 0x05:## 打开客厅灯光
                print("打开客厅灯光")
                self.init_label(self.living_light_label, "开启", font=10, color="black")
            elif node == 0x06:## 关闭客厅灯光
                print("关闭客厅灯光")
                self.init_label(self.living_light_label, "关闭", font=10, color="black")
             
             
            elif node == 0x09:## 打开书房灯光
                print("打开书房灯光")
                self.init_label(self.study_light_label, "开启", font=10, color="black")
            elif node == 0x0A:## 关闭书房灯光
                print("关闭书房灯光")
                self.init_label(self.study_light_label, "关闭", font=10, color="black")
            ...
            if node == 0x01:## MP3播放
                print("MP3播放")
                self.init_label(self.music_name_label, "播放", font=10, color="black")
                self.Load_pic(self.music_label, PLAY_IMAGE)
            elif node == 0x02:## 暂停mp3
                print("暂停mp3")
                self.init_label(self.music_name_label, "暂停", font=10, color="black")
                self.Load_pic(self.music_label, STOP_IMAGE)
            elif node == 0x03:## 继续播放
                print("继续播放")
                self.init_label(self.music_name_label, "播放", font=10, color="black")
                ...
                print("停止播放")
                self.init_label(self.music_name_label, "停止", font=10, color="black")
                self.Load_pic(self.music_label, STOP_IMAGE)
            elif node == 0x05:## 播放下一首
                print("播放下一首")
                self.init_label(self.music_name_label, "播放下一首", font=10, color="black")
                self.Load_pic(self.music_label, PLAY_IMAGE)
                time.sleep(1)
                self.init_label(self.music_name_label, "播放", font=10, color="black")
            elif node == 0x06:## 播放上一首
                print("播放上一首")
                self.init_label(self.music_name_label, "播放上一首", font=10, color="black")
                self.Load_pic(self.music_label, PLAY_IMAGE)
                time.sleep(1)
                self.init_label(self.music_name_label, "播放", font=10, color="black")
            elif node == 0x07:## 音量调高
                ...
                print("调高书桌")
                self.init_label(self.study_desk_label, "升高", font=10, color="black")
                time.sleep(1)
                self.init_label(self.study_desk_label, "高", font=10, color="black")
            elif node == 0x0D:## 调低书桌
                print("调低书桌")
                self.init_label(self.study_desk_label, "降低", font=10, color="black")
                time.sleep(1)
                self.init_label(self.study_desk_label, "低", font=10, color="black")
            elif node == 0x0E:## 停止书桌
                print("停止书桌")   
                self.init_label(self.study_desk_label, "停止", font=10, color="black")
            elif node == 0x0F:## 打开排风
                print("打开排风")
                auto_fan=False
                self.Load_pic(self.bathroom_fan_label, ON_IMAGE)
    ...
    def update_xiaozhi_display(self,living_tmp,living_hum,ba_tmp,ba_hum,smoke,door_status,hood_status,MP3_Name):
        ...
        self.init_label(self.kitchen_smoke_label,f"{smoke}", font=10, color="black")
        ...
        self.init_label(self.music_name_label, f"{MP3_Name}", font=10, color="black")
    ...
    def handle_blinker_message(self,control_key,command): 
        ...
        if control_key == "led01":           
            ...
            if command == "on":
                    ...
                    self.init_label(self.hostroom_curtain_label, "开启", font=10, color="black")
                    print("主卧窗帘已开启")
            elif command == "off":
                    send_lora_command(0x03, 0x02, 0x00)
                    self.init_label(self.hostroom_curtain_label, "关闭", font=10, color="black")
                    print("主卧窗帘已关闭")
            ...
            if command == "tap":
                    ...
                    self.init_label(self.study_desk_label, "上升", font=10, color="black")
                    print("书桌已上升")
        elif control_key == "desk02":
            if command == "tap":
               
                send_lora_command(0x02, 0x0D, 0x00)
                self.init_label(self.study_desk_label, "下降", font=10, color="black")
                print("书桌已下降")

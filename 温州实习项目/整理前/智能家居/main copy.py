...
class BlinkerWorker(QObject):
    ...
    blinker_msg_signal = pyqtSignal(list)
    ...
    def monitor_data_reader(self):
        """步骤2：检测Blinker消息队列，通过信号发送到主线程（主线程调用）"""
        try:
            # 检查Blinker设备的消息队列是否有数据
            if hasattr(device, 'data_reader') and not device.data_reader.empty():
                raw_msg = device.data_reader.get_nowait()
                # 正确解析Blinker消息（格式：{"数据": {"设备名": "指令"}}）
                fromDevice = raw_msg.get("fromDevice", "")
                data_dict = raw_msg.get("data", {})
                if data_dict:
                    # 提取第一个（或所有）设备-指令对（根据实际需求调整）
                    for device_key, command_val in data_dict.items():
                        msg_list = [fromDevice, device_key, command_val]
                        # print(f"[Blinker] 提取消息: {msg_list}")
                        # 通过信号发送到主线程（自动排队，线程安全）
                        self.blinker_msg_signal.emit(msg_list)
                        # 发送到串口
                         
                        # 调用通用消息处理器（异步）
                        if self.blinker_loop:
                            asyncio.run_coroutine_threadsafe(
                                general_message_handler(raw_msg), 
                                self.blinker_loop
                            )
        except Exception as e:
            print(f"[错误] 处理Blinker消息时出错: {e}")
    ...
    def run(self):
        ...
        try:
            ...
            async def realtime_func(keys):
                """实时数据请求处理函数"""
                print(f"收到实时数据请求: {keys}")
                try:
                    for key in keys:
                        if key == "temp01":
                            def get_temp01():
                                return self.cached_living_tmp
                            await device.sendRtData(key, get_temp01)
                            print(f"发送实时温度数据: {self.cached_living_tmp}°C")
                        elif key == "hum01":
                            def get_hum01():
                                return self.cached_living_hum
                            await device.sendRtData(key, get_hum01)
                            print(f"发送实时湿度数据: {self.cached_living_hum}%")
                        elif key == "temp02":
                            def get_temp02():
                                return self.cached_ba_tmp
                            await device.sendRtData(key, get_temp02)
                            print(f"发送实时浴室温度: {self.cached_ba_tmp}°C")
                        elif key == "hum02":
                            def get_hum02():
                                return self.cached_ba_hum
                            await device.sendRtData(key, get_hum02)
                            print(f"发送实时浴室湿度: {self.cached_ba_hum}%")
                except Exception as e:
                    print(f"实时数据处理错误: {e}")
            
            # 创建设备实例，设置实时数据处理函数
            device = Device("f9094bf7c991", heartbeat_func=heartbeat_func, realtime_func=realtime_func)
            self.device = device  # 存储设备引用
            
            # 添加控件
            temp01 = device.addWidget(NumberWidget('temp01'))  # 客厅温度
            hum01 = device.addWidget(NumberWidget('hum01'))    # 客厅湿度
            temp02 = device.addWidget(NumberWidget('temp02'))  # 浴室温度
            hum02 = device.addWidget(NumberWidget('hum02'))    # 浴室湿度
                 ...
                 while self._is_running:
                     try:
                         ...
                         if self._is_running:
                             ...
                             await device.sendRtData("temp01", get_temp01)
                             await device.sendRtData("hum01", get_hum01)
                             await device.sendRtData("temp02", get_temp02)
                             await device.sendRtData("hum02", get_hum02)
                             print(f"定时发送数据到App: 客厅温度{self.cached_living_tmp}°C, 客厅湿度{self.cached_living_hum}%, 浴室温度{self.cached_ba_tmp}°C, 浴室湿度{self.cached_ba_hum}%")
                     except Exception as e:
                         print(f"定时更新数据时出错: {e}")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, blinker_worker=None):
        super().__init__()        
        uic.loadUi('E:\Arduinoproject\智能家居\main.ui', self)##加载UI文件
    ...
    def update_xiaozhi_display(self,living_tmp,living_hum,ba_tmp,ba_hum,smoke,door_status,hood_status,MP3_Name):
        ...
        ###把living_tmp living_hum ba_tmp ba_hum smoke 发送给app端
    ...
    def handle_blinker_message(self,msg): 
        ...
        control_key = msg[1]
        command = msg[2]
        print(f"主线程Blinker消息: 设备: {device_name}, 控件: {control_key}, 指令: {command}")
        
        # 处理不同设备的按钮消息
        if control_key == "led01":           
            if command == "on":
                # 更新UI为打开状态
                if self.hostroom_light_label.text() != "开启":
                    send_lora_command(0x01, 0x01, 0x00)
                    self.init_label(self.hostroom_light_label, "开启", font=9, color="black")
                    print("主机房间灯已开启")
                elif self.hostroom_light_label.text() != "关闭":
            ...
            if command == "on":
                # 更新UI为打开状态
                if self.secound_light_label.text() != "开启":
                    send_lora_command(0x01, 0x03, 0x00)
                    self.init_label(self.secound_light_label, "开启", font=9, color="black")
                    print("次卧灯光已开启")
                elif self.secound_light_label.text() != "关闭":
            ...
            if command == "on":
                # 更新UI为打开状态
                if self.living_light_label.text() != "开启":
                    send_lora_command(0x01, 0x05, 0x00)
                    self.init_label(self.living_light_label, "开启", font=9, color="black")
                    print("客厅灯光已开启")
                elif self.living_light_label.text() != "关闭":
            ...
            if command == "on":
                ...
                if blink_hood != True:
                    ...
                    self.Load_pic(self.kitchen_hood_label, ON_IMAGE)
                    print("厨房 Hood 已开启")
                elif blink_hood != False:
                    blink_hood=False
                    send_lora_command(0x03, 0x07, 0x00)
                    self.Load_pic(self.kitchen_hood_label, OFF_IMAGE)
                    print("厨房 Hood 已关闭")
        elif control_key == "fin02":
            if command == "on":
                global blink_fan
                if blink_fan != True:
                    blink_fan=True
            ...
            if command == "on":
                global blink_window
                if blink_window != True:
                    blink_window=True
                    send_lora_command(0x03, 0x01, 0x00)
                    self.Load_pic(self.living_window_label, ON_IMAGE)
                    print("客厅窗口已开启")
                elif blink_window != False:
                    blink_window=False
            ...
            if command == "on":
                global blink_curtain
                if blink_curtain != True:
                    blink_curtain=True
                    send_lora_command(0x02, 0x13, 0x00)
                    self.Load_pic(self.secound_curtain_label, ON_IMAGE)
                    print("次卧窗帘已开启")
                elif blink_curtain != False:
                    blink_curtain=False

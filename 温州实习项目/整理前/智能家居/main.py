    ...
    def stop(self):
        ...
        if self.blinker_loop and not self.blinker_loop.is_closed():
            try:
                ...
                print("Blinker事件循环已停止")
            except Exception as e:
                print(f"停止Blinker事件循环时出错: {e}")
        ...
        try:
            ...
            async def wind03_key_handler(msg):
                if msg['wind03']=='on':             
                    ...
                    await wind03_key.turn('on').color('#FF8C00').text('客厅窗帘打开').update()
                elif msg['wind03']=='off':                    
                    self.blinker_msg_signal.emit("wind03","off")
                    await wind03_key.turn('off').color('#101010').text('客厅窗帘已关闭').update()

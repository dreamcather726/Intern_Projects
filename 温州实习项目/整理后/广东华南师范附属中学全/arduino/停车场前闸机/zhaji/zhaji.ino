...
*/
volatile int count;
volatile boolean validTagFound;
volatile int tagLabel;
...
void page2() {
  ...
  for (int i = (1); i <= (count); i = i + (1)) {
    if (sentry.GetValue(Sentry2::kVisionAprilTag,kLabel,i) < 3) {
...
void loop(){
  ...
  if (count > 0) {
    for (int i = (1); i <= (count); i = i + (1)) {
      tagLabel = sentry.GetValue(Sentry2::kVisionAprilTag,kLabel,i);
      if (tagLabel >= 0 && tagLabel <= 3) {
        validTagFound = true;
        chepaihao();
        controlZhaji(1);

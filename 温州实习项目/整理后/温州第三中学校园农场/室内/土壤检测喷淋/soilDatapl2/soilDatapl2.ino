...
// ========== 设备配置区域 ==========
#define DEVICE_ID     5     // 设备ID（室内设备为4、5、6）
...
const unsigned long RELAY_ON_TIME = 5000; // 继电器开启时间（5秒）
...
void handleControlCommand() {
  ...
  if (Serial.available() >= 4) {
    ...
    byte data = buffer[2];
    byte frameEnd = buffer[3];
    
    // 检查是否是发送给当前设备的指令
    bool isTargetDevice = (deviceId == DEVICE_ID) || (deviceId == 0x66); // 0x66为广播地址
    bool isCommandValid = (frameEnd == 0xFF && data == 0x01);           // 帧尾有效且数据为1
    
    // 执行继电器操作
    if (isTargetDevice && isCommandValid) {
      activateRelay();
    }
  }
}
